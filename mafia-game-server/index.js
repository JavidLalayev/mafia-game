...
io.on("connection", (socket) => {
  ...
  socket.on("mafia_message_send", ({ sender, msg, pictureUrl, socketId }) => {
    ...
    if(mafiaWriters.some(writer => writer.writerId === socketId)){
    ...
    io.emit("mafia_message_send", { sender: sender, msg: msg, pictureUrl: pictureUrl, socketId: socketId, writers: mafiaWriters });
  });
  ...
  socket.on("mafiaNoLongerWriting", ({id, username}) =>{
    ...
    io.emit("mafiaWriting", mafiaWriters)
  ...
  socket.on("mafiaOnWriting", ({id, username, pictureUrl}) =>{
  ...
  socket.on("distributeUser", async () => {
    ...
    if (clients.length >= 5) {
      ...
      io.emit("isGameStart", isGameStart);
  ...
  socket.on("socketChooseMafia", ({senderId, username, choosenId}) => {
    ...
    if(players.length == civilChoose.length){
      ...
      msg.push({
        for: roles.all,
        content: "Vətəndaşların seçimi nəticəsində " + deadPlayer.username + " öldürüldü. O " + deadPlayer.role + " idi",
      });
      io.emit("executeMessages", { game_info: msg });
...
function distrubuteRoles() {
  var mafiaCount = Math.floor((clients.length / 5) * 2);
  isDay = false;
  var randomNumbers = [];
  ...
  var onlyPlayers = players.filter(comparer(clients));
  var onlyClients = clients.filter(comparer(players));
    ...
    if(isDay == false){
      ...
      if((players.length > 2 && mafias.length > playersWithoutMafia.length) || (players.length <= 2 && mafias.length >= playersWithoutMafia.length)){
        ...
        game_info_storage.push({
          for: roles.all,
          content: "Günəş batmaqa başlayır",
        });

        if(players.some(player => player.role == roles.doctor)){
          game_info_storage.push({
            for: roles.all,
        ...
        io.emit("dayChange", {players: players, deadPlayers: deadPlayers, game_info: game_info_storage, isDay: isDay});
...
function mafiaWin(){
  ...
  players.forEach(player => {
    if(player.role == roles.don || player.role == roles.mafia){
      winners += player.username  + ' ' + player.role + ' idi \n'
    }else{
      losers += player.username  + ' ' +  player.role + ' idi \n'
    }
  });
  ...
  lastPlayers.forEach(lastPlayer => {
    if(!players.some(player => player.id == lastPlayer.id)){
      losers += lastPlayer.username  + ' ' +  lastPlayer.role + ' idi \n'
    }
  });
  ...
  io.emit("gameFinish", {game_info: finish_info_storage});
...
function civilWin(){
  ...
  finish_info_storage.push({
    for: roles.all,
    content: "Oyun bitdi vətəndaşlar qazandi",
  });
...
function findDeadPlayer(){
  lastPlayers.forEach(lastPlayer => {
    if(!players.some(player => player.id == lastPlayer.id)){
      deadPlayers.push(lastPlayer);
    }
  });
}
